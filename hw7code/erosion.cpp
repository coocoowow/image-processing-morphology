...
cv::Mat erosion(Mat ori,int masksize)
{
	...
	for (int i = 0; i<out.rows; i++)
	{
		for (int j = 0; j<out.cols; j++)
		{
			...
			for(int k =-halfmasksize;k<halfmasksize;k++)
			{
				...
				{
					if(ori_mirror.at<uchar>(i+halfmasksize+k,j+halfmasksize+d)<temp)
					{
						temp=ori_mirror.at<uchar>(i+halfmasksize+k,j+halfmasksize+d);
					}
